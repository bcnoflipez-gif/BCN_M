...
export default function MobileLayout({ children }: MobileLayoutProps) {
  return (
    <div suppressHydrationWarning className="h-[100dvh] w-full bg-black flex justify-center items-stretch font-sans antialiased select-none overflow-hidden">
      ...
      */}
      <div suppressHydrationWarning className="w-full max-w-md bg-[var(--background)] text-[var(--foreground)] border-x border-[var(--border)] shadow-2xl relative flex flex-col h-full overflow-hidden transition-colors duration-300">
        {children}
      </div>
    </div>
  );
